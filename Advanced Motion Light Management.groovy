...
def pageSetup() {
    ...
    return dynamicPage(pageProperties) {
        ...
        section()
        {
            input "watchdog", "bool", title: "Use this app as a watchdog (reboots the hub when it becomes unresponsive)", defaultValue: false
        }
        section() {
...
def initialize() {
    ...
    for(s!=0;i<s;i++)
    {
        subscribe(switches[i], "motion", switchHandler)
        log.trace "${switches[i]} subscribed to events"
    }
    if(contacts)
    {
        i = 0
        ...
        for(s!=0;i<s;i++)
        {
            ...
            log.trace "${contacts[i]} subscribed to events"
        }
    }
...
{
    logging("$evt.device is now set to $evt.value")
}
...
def master(){
    ...
    {
        if(now() - state.lastRun > state.timer * 3 * 60000) // if this loop hasn't run for more than the timer's value * 3, then it's probably that CRON is broken
        {
            log.warn "CRON SERVICE BROKEN!"

            // first, try to update the app
            if(state.fix < 2) 
            {
                log.warn "REINITIALIZING $app.label" 
                updated()
                state.fix += 1
            }
            else if(watchdog && now() - state.lastReboot > 15 * 60 * 1000)
            {
